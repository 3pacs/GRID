    ...
    try:
        ...
        if client and client.is_available:
            prompt = (
                ...
                "For each divergence, identify WHO is causing it (the lever puller) and WHY "
                "(which valve they are operating — e.g. fiscal spending, rate policy, reserve "
                "requirements, reporting methodology). Statistical divergence alone is not a cause — "
                "name the actor and the mechanism. Separate the LEVER (actor + action + valve) from "
                "CONDITIONS (market environment that amplifies the divergence).\n\n"
