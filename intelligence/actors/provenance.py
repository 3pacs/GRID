...
that can represent all three states above, including ``'unconfirmed'``.
Every current call site that labels an actor node selects and passes this
column; there is no live caller left that relies on the membership-only
fallback below. The fallback still exists for a hypothetical future call
site that truly cannot select the row (id only, no DB access in scope), and
is deliberately conservative about what it can claim: it can only ever
return ``PROVENANCE_SEED`` or ``PROVENANCE_OBSERVED`` (a bare id carries no
signal that would justify guessing ``'unconfirmed'``), so a caller using it
for a seed id that has actually been modified would report stale
``'curated_seed'`` — call sites that can select the column must do so rather
than accept this approximation.
"""
