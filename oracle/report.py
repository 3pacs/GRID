...
def send_oracle_report(cycle_result: dict[str, Any]) -> None:
    ...
    # ── Send ────────────────────────────────────────────────────────────
    # Distinct positions written this cycle, not model votes: the dedup upsert
    # collapses N models x M tickers to one row per (ticker, direction, expiry,
    # type, day). "4,340 predictions" in a subject line meant 14 positions.
    n_votes = cycle_result.get("new_predictions", 0)
    n_preds = cycle_result.get("distinct_positions_written", n_votes)
    subject = (
        f"GRID Oracle — {n_preds} Positions ({n_votes} model votes) | Cycle Report"
        if n_preds != n_votes else f"GRID Oracle — {n_preds} Predictions | Cycle Report"
    )
        ...
        if settings.REF_CHECK_ENABLED:
            ...
            if refs:
                ...
                try:
                    ...
                    loop.close()
                annotated = annotate_output(plain, url_results)
                plain = annotated.cleaned_text
                # Re-render HTML with cleaned text if refs were modified
                if annotated.removed_count or annotated.replaced_count:
                    html = _render_html(subject, sections,
                                        footer_note="Predictions are generated by the self-improving Oracle engine. "
                                                    "Scored after expiry. Model weights evolve with track record.")
                log.info(
                    "Ref guard on Oracle report: {rem} removed, {rep} replaced",
                    rem=annotated.removed_count, rep=annotated.replaced_count,
                )
    except Exception as exc:
        log.debug("Reference check skipped for Oracle report: {e}", e=str(exc))
