...
def _verdict_call(buckets: dict[str, BucketStats]) -> str:
    ...
    if hr_lift_pp >= 5.0 and pnl_lift_pp > 0:
        return f"CALIBRATED — HIGH beats MEDIUM by {hr_lift_pp:.1f}pp hit-or-partial and {pnl_lift_pp:+.2f}% mean PnL"
    if hr_lift_pp <= -5.0 or pnl_lift_pp < 0:
        return f"BROKEN — HIGH underperforms MEDIUM ({hr_lift_pp:+.1f}pp hit-or-partial, {pnl_lift_pp:+.2f}% PnL)"
