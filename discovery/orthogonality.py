...
class OrthogonalityAudit:
    ...
    def run_full_audit(
        ...
        #
        # Embeddings resolve through EMBED_PROVIDER_CHAIN (tailnet GPU Ollama
        # nodes), not the Hyperspace node — so this no longer gates on
        # Hyperspace being up. It used to, which meant that once the CPU-only
        # :8080 unit went away this whole block would have been skipped.
        ...
        try:
            ...
            log.debug(
                "Semantic similarity skipped: {e}", e=str(exc)
            )
