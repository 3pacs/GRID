...
def _prediction_text_corpus(req: AstrogridPredictionRequest) -> str:
    return " ".join(
        part
        for part in [
            req.question,
...
def _overlay_scorecard_symbols(
    ...
    *,
    ...
    if not isinstance(scorecard, dict):
        return symbols
    for bucket in ("items", "leaders", "laggards"):
        for item in list(scorecard.get(bucket) or [])[:6]:
...
def _infer_prediction_horizon(req: AstrogridPredictionRequest) -> str:
    ...
    if explicit in {"macro", "swing"}:
        return explicit
    text_corpus = _prediction_text_corpus(req)
    if any(
...
def _infer_target_symbols(req: AstrogridPredictionRequest) -> list[str]:
    if req.target_symbols:
        return list(dict.fromkeys(str(symbol).upper() for symbol in req.target_symbols[:12]))
    text_corpus = _prediction_text_corpus(req)

    matched_symbols: list[tuple[int, str]] = []
    for symbol, aliases in _ASSET_ALIASES.items():
        best_match: int | None = None
        for alias in aliases:
            match = re.search(rf"(?<![a-z0-9]){re.escape(alias)}(?![a-z0-9])", text_corpus)
            if match and (best_match is None or match.start() < best_match):
                best_match = match.start()
        if best_match is not None:
            matched_symbols.append((best_match, symbol))
    if matched_symbols:
        return [symbol for _, symbol in sorted(matched_symbols)][:12]
    ...
    universe_hint = str(req.target_universe or "").strip().lower()
    if group_filter is None and universe_hint in {"crypto", "equity", "macro"}:
        group_filter = universe_hint

    symbols = _overlay_scorecard_symbols(req.market_overlay_snapshot or {}, group_filter=group_filter)
    if symbols:
...
def _infer_target_group(target_symbols: list[str], req: AstrogridPredictionRequest) -> str:
    ...
    for group_name, cues in _GROUP_CUE_MAP.items():
        if any(
            ...
            return group_name
    universe_hint = str(req.target_universe or "").strip().lower()
    if universe_hint in {"crypto", "equity", "macro"}:
...
def _infer_question_intent(
    ...
    ) or ("when" in text_corpus and "buy" in text_corpus):
        return "timing_entry"
    if "wait" in text_corpus and "buy" in text_corpus:
        return "buy_or_wait"
    if len(target_symbols) > 1 and any(
        cue in text_corpus for cue in (" or ", "/", " vs ", " versus ", " between ")
    ):
...
def _build_postmortem_stub(req: AstrogridPredictionRequest) -> dict[str, Any]:
    ...
    invalidation = (req.invalidation or "").strip()
    lowered_invalidation = invalidation.lower()
    invalidation_clause = invalidation
    for prefix in (
        "exit if ",
        "break if ",
        "cancel if ",
        "cancel the entry if ",
        "cancel the long setup if ",
        "break the avoid call only if ",
    ):
        if lowered_invalidation.startswith(prefix):
            invalidation_clause = invalidation[len(prefix):]
            break
    summary = (
        f"Pending {horizon} read on "
        f"{', '.join(target_symbols) if target_symbols else req.target_universe}: "
        f"{req.call}. "
        f"Invalid if {invalidation_clause.rstrip('.')}."
    )
    return {
        "summary": summary,
...
def _classify_prediction_scoreability(
    ...
    from astrogrid_api.dependencies import get_db_engine
