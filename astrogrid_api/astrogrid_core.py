...
from astrogrid_api.auth import require_auth
from astrogrid_api.dependencies import get_astrogrid_store, get_db_engine
from astrogrid_api.observability import APP_TAG, get_tracer
...
async def interpret_snapshot(
    ...
    try:
        ...
        tracer = get_tracer()
        gen_metadata = {
            "feature": "astrogrid-interpret-snapshot",
            "backend": backend,
            "model": model,
            "temperature": 0.2,
            "num_predict": 1200,
            "message_count": len(messages),
            "app_env": tracer.app_env,
        }
        with tracer.start_generation(
            name="astrogrid.interpret_snapshot",
            model=model or backend or "unknown",
            input={"messages": messages} if tracer.capture_io else None,
            metadata=gen_metadata,
            extra_tags=(APP_TAG, "feature:interpret_snapshot"),
        ) as generation:
            raw = client.chat(
                messages=messages,
                temperature=0.2,
