...
The two directories must exist as canonical, direct children of the release root. The scheduler directory must match the running `grid-scheduler` PID's resolved cwd and Git HEAD, and the effective systemd `WorkingDirectory` must be that exact immutable directory. The script checks all of this while holding its deploy lock. It exits 5 before the build on a missing or invalid record, changed process, or mutable unit path. It retains both directories through every prune pass regardless of mtime, in addition to the candidate and immediate previous release.
