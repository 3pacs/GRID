...
- Fixed dead quick actions: command palette options scan now uses the GET scanner, command palette [[Source Audit|source audit]] now posts to `/source-audit/run`, and mobile pipeline now triggers enabled workflow run endpoints instead of nonexistent `/workflows/run-all`.
