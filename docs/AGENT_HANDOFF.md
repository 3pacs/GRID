## 2026-07-17 23:20 UTC — 2026-07-17-2308
**Why this matters next run:** PUNCH-LIST line 173 (scripts/download_pm_data.py:65 shell=True zstd|tar pipe) shipped as PR #377. Argv-form `Popen`(zstd) → `run`(tar) with explicit non-zero-exit checks on both. Strike line 173 before re-picking. First scripts/-section P1 to close; the other three script-security items in that section are still open (age_fast_sync Cypher `_esc`, deploy.py tests, verify_systemd_units.py tests, dispatch_agent.py tests).

- **TIER 0 status:** GRID Tests **GREEN** on main HEAD `f09cbb86`. "Deploy to Grid Server" workflow **FAILED** on the same SHA — this is the infra/SSH deploy step, not a code failure (routine agents have no SSH per North Star). Do NOT try to "fix" it from code; leave for the operator's local session. Confirmed pattern: several recent SHAs show Deploy=success while others show Deploy=failure with GRID Tests green throughout.
- **Env note (works cleanly this run):** the base container ships without `pytest`, `pandas`, `sqlalchemy`, `loguru`, or `ruff`. `python3 -m pip install pytest pandas sqlalchemy loguru ruff -q` gets you running in ~15s. Once installed, `python3 -m pytest tests/<file> -x -q` works without `--noconftest`. This is a simpler path than the 2026-06-30 handoff's uv-managed pytest recipe.
- **PUNCH-LIST staleness is still real** (still worth a docs-only reconciliation PR): the 2026-06-30 handoff called out that lines 88, 108, 109, 110, 127, 128, 130, 131 are already covered by test files but still show `[ ]`. As of this run they are STILL not flipped. Also: my run's PR did not flip line 173 to `[x]` either (kept scope tight), so add that to the reconciliation stack. Everything the last 4 handoffs said was "still pickable" from lines 94, 96, 117, 118 remains valid.
- **Small P1 items still pickable in the scripts/ section** (2026-07-09 auditor block, PUNCH-LIST lines 174–177): each is a self-contained 1-file security fix or 1-file test add. Priority order after this run: line 174 (age_fast_sync Cypher `_esc` doesn't neutralize `$$`), line 173 DONE, then the three test-add items (deploy.py / verify_systemd_units.py / dispatch_agent.py). None conflict with any open PR or agent branch as of 23:20 UTC.
- **Tool-result truncation still hits** `mcp__github__list_pull_requests` (142k chars, 11 PRs) and `mcp__github__actions_list` (338k chars, 30 runs). Save-to-file + `python3 -c "import json; ..."` on the returned file path is the canonical workaround.
