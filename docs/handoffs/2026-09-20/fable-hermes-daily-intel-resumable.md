...
## Live-scheduler reconciliation (read-only check on grid-svr, 2026-09-20 06:59–07:00Z) — supersedes the repository-only search

The repository invocation search above was insufficient: **`grid-flow-materializer.timer` exists and is enabled and active on grid-svr** (`/etc/systemd/system/grid-flow-materializer.timer`, file dated 2026-05-17; service file 2026-07-13; `hc-ping.conf` drop-in 2026-06-01; `OnBootSec=5min`, `OnUnitActiveSec=30min`, `Persistent=true`; last trigger 06:34:27Z, next 07:04:34Z). Its service is a oneshot as user `grid` with `WorkingDirectory=/home/grid/grid_v4/grid_repo` (the old checkout) and `ExecStart=python3 -c 'import sys; sys.path.insert(0, "/home/grid/grid_v4/grid_repo"); from ingestion.flow_materializer import sync_all; …; print(sync_all(get_engine()))'` plus a healthcheck ping; the 06:34Z run succeeded (insider_trades 5000, congressional_trades 724, …). The old-tree `ingestion/flow_materializer.py` has no local modifications and is byte-identical to the release tree (last commit 2026-06-25). The unit never disappeared; it is not tracked under `server_setup/`, which is why the repo search missed it. History of its creation/edits beyond the file mtimes is unknown.

A sweep of every timer, every unit `ExecStart` and every crontab on the host found **no other live invocation** of the seven remaining enabled functions.

**Consequence:** `flow_materialize` has a second automated writer in a separate process on the same incremental keys, which `_DAILY_INTEL_IN_FLIGHT` cannot coordinate → **held** for the initial subset (allow-list now 7; holds 14). Coordination proposal, not implemented: the timer already materialises every 30 minutes, so the Hermes daily task is redundant; if it is ever enabled, it needs cross-process coordination (e.g. a DB advisory lock shared with the timer's service) rather than the in-process registry.

**Stale-write guarantee, restated:** full recomputation repairs a stale value only after the next successful run — there is no guaranteed elapsed-time bound (a run may time out or be skipped for the period). Incremental writers may leave a stale key indefinitely once it has left their window.
