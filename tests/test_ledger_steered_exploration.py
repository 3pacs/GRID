...
def new_ledger(tmp_path=None, q=0.10):
    path = None if tmp_path is None else tmp_path / "ledger.jsonl"
    return lse.Ledger.create(path, ledger_id="s11-test", q=q, recorded_at=FIXED)
...
def test_ledger_is_append_only_and_hash_chained(tmp_path):
    ...
    assert lse.Ledger(path).head == ledger.head  # reload verifies the chain

    with pytest.raises(FileExistsError):
        new_ledger(tmp_path)  # a ledger is never overwritten

    original = path.read_bytes()
    path.write_bytes(original.replace(b'"run_id":"r1"', b'"run_id":"r9"'))
    with pytest.raises(ValueError, match="chain"):
        lse.Ledger(path)
    # an edit of the last record is invisible to the chain: anchor the head
    path.write_bytes(original.replace(b'"reason":"test"', b'"reason":"tost"'))
    lse.Ledger(path)
    with pytest.raises(ValueError, match="expected head"):
        lse.Ledger(path, expected_head=ledger.head)
    ...
    with pytest.raises(ValueError, match="chain"):
        lse.Ledger(path)
    path.write_bytes(original[:-1])  # truncated
...
def test_genesis_freezes_the_global_level():
    with pytest.raises(ValueError):
        lse.Ledger.create(None, ledger_id="x", q=0.2)
    with pytest.raises(ValueError):
        lse.Ledger.create(None, ledger_id="", q=0.1)
...
def test_excluded_telemetry_never_allocated():
    ...
    allocation = lse.allocate(
        new_ledger(), catalog, lse.Policy(budget=10), run_id="r", windows=window(1),
        recorded_at=FIXED,
    )
...
def test_holdout_windows_are_single_use_per_family():
    ledger = new_ledger()
    catalog = lse.synthetic_catalog()
    policy = lse.Policy(budget=8)  # floor only: one trial per arm
    first = lse.allocate(ledger, catalog, policy, run_id="r1", windows=window(1),
                         recorded_at=FIXED)
    touched = {t["family_key"] for t in first["record"]["trials"]}
    lse.abandon(ledger, "abandoned runs still consume their windows", recorded_at=FIXED)
    overlapping = dict(window(1), start=window(1)["split"],
                       end=(pd.Timestamp(window(1)["end"]) + pd.Timedelta(days=90)).isoformat(),
                       split=window(1)["end"])
    with pytest.raises(ValueError, match="no eligible family"):
        lse.allocate(ledger, catalog, policy, run_id="r2", windows=overlapping,
                     recorded_at=FIXED)
    fresh = lse.allocate(ledger, catalog, policy, run_id="r3", windows=window(2),
                         recorded_at=FIXED)
    assert {t["family_key"] for t in fresh["record"]["trials"]} == touched
    lse.abandon(ledger, "x", recorded_at=FIXED)

    # a partly-touched window: only untouched families remain eligible
    narrow = lse.Ledger.create(None, ledger_id="narrow", recorded_at=FIXED)
    only_alpha = lse.Catalog(
        families=catalog.families, features=catalog.features, classes=catalog.classes,
        self_lag=catalog.self_lag + tuple(
            ("T2|change|fwd1", f) for f in catalog.features if not f.startswith("alpha")
        ) + tuple(
            ("T1|change|fwd1", f) for f in catalog.features
            if not f.startswith("alpha") and f != lse.SYNTHETIC_OWN
        ),
    )
    lse.allocate(narrow, only_alpha, policy, run_id="a", windows=window(1),
                 recorded_at=FIXED)
    lse.abandon(narrow, "x", recorded_at=FIXED)
    second = lse.allocate(narrow, catalog, policy, run_id="b", windows=window(1),
                          recorded_at=FIXED)
    arms = second["record"]["arms"]
    assert arms[PLANTED]["eligible"] is False
    assert "no fresh window" in arms[PLANTED]["ineligible"]
    assert arms["beta::T1|change|fwd1"]["eligible"] is True
    assert PLANTED not in {t["family_key"] for t in second["record"]["trials"]}
...
def test_record_run_refuses_anything_but_the_allocated_run(tmp_path):
    ...
    assert lse.Ledger(tmp_path / "ledger.jsonl").head == ledger.head
...
def test_two_dry_runs_shift_budget_to_the_planted_family(tmp_path):
    ...
    ledger = lse.Ledger(tmp_path / "dry" / "ledger.jsonl")
...
def test_twenty_noise_runs_end_to_end_accumulate_no_false_discoveries(tmp_path):
    ...
    ledger = lse.Ledger(tmp_path / "noise" / "ledger.jsonl")
    ...
    # the same recorded p-values under per-run BH at q (no cross-run accounting)
    naive = sum(
        sum(a <= 0.10 for a in bh_adjusted([t["p"] for t in r["trials"]]))
        for r in results
    )
    assert naive > 0  # the leak the ledger closes
...
def simulate(rng, runs, q, rho, planted_shift=0.0):
    ...
    ledger = lse.Ledger.create(None, ledger_id="mc", q=q, recorded_at=FIXED)
    catalog = lse.synthetic_catalog()
...
def test_forward_outcomes_feed_the_ledger(tmp_path):
    ...
    path = tmp_path / "dry" / "ledger.jsonl"
    ledger = lse.Ledger(path)
    ...
    for bad in (
        verdict(losers[0], candidate_sha256=None),
        verdict(survivors[0], candidate_sha256="c" * 64),
        verdict(survivors[0], outcome="pending"),
        verdict(survivors[0], n=0),
        json.dumps({"trial_id": survivors[0]["trial_id"]}),
    ):
        with pytest.raises(ValueError):
            lse.ingest_forward_outcomes(ledger, write("bad.jsonl", bad))
    with pytest.raises(ValueError, match="already"):
        lse.ingest_forward_outcomes(
            ledger, write("dup.jsonl", verdict(survivors[0]), verdict(survivors[0]))
        )
    assert ledger.of_kind("forward_outcome") == []  # all-or-nothing

    source = write("fwd.jsonl", verdict(survivors[0]),
                   verdict(survivors[1], outcome="fail"))
    assert lse.ingest_forward_outcomes(ledger, source, recorded_at=FIXED) == 2
    ...
    with pytest.raises(ValueError, match="already"):
        lse.ingest_forward_outcomes(ledger, write("again.jsonl", verdict(survivors[0])))
    assert lse.Ledger(path).head == ledger.head
    assert lse.summarize(ledger)["forward_verdicts"] == {"pass": 1, "fail": 1}
