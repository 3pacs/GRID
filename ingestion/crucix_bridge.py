...
class CrucixBridgePuller(BasePuller):
    ...
    def __init__(
        ...
        self.crucix_url = crucix_url or "http://localhost:3117/api/data"
    ...
    def _load_latest(self) -> dict[str, Any]:
        """Load Crucix raw data from file (faster, for numeric signals)."""
    ...
    def _load_from_http(self) -> dict[str, Any]:
        """Load Crucix data from HTTP API (includes news, ideas, delta)."""
        try:
            ...
            log.error("Failed to load Crucix data from HTTP: {e}", e=str(exc))
            return {}

    def _load_enriched(self) -> dict[str, Any]:
        """Load enriched Crucix data (news, ideas, delta) from HTTP API.

        The file-based latest.json only has raw source data.
        News feed, trade ideas, and delta signals are generated by the
        dashboard injection layer and only available via HTTP.
        """
        return self._load_from_http()
    ...
    def pull_all(self) -> dict[str, Any]:
        ...
        # ── Ingest enriched data (news, ideas, delta) from HTTP API ──
        enriched = self._load_enriched()
        news_inserted = 0
        signals_inserted = 0
        if enriched:
            with self.engine.begin() as conn2:
                news_inserted = self._ingest_news(enriched, conn2)
                signals_inserted = self._ingest_ideas_and_delta(enriched, conn2)

        log.info(
            "Crucix bridge — {ins} series + {news} news + {sig} signals from {src} sources",
            ins=total_inserted,
            news=news_inserted,
            sig=signals_inserted,
    ...
    def _ingest_news(self, data: dict, conn: Any) -> int:
        """Ingest Crucix news feed into news_articles table.

        Deduplicates by URL hash. Extracts headline, source, region as tickers proxy.
        """
        import hashlib

        news_items = data.get("newsFeed", data.get("news", []))
        if not news_items:
            return 0
        ...
        for item in news_items:
            ...
            source = item.get("source", "Crucix")
            region = item.get("region", "")
            urgent = item.get("urgent", False)
            timestamp_str = item.get("timestamp", item.get("date", ""))

            if not headline or not url:
                continue

            dedup_hash = hashlib.sha256(url.encode()).hexdigest()[:32]
            ...
            if timestamp_str:
                try:
                    ...
                    published_at = parsedate_to_datetime(timestamp_str)
                except Exception:
                    try:
            ...
            # Map region to tickers array for searchability
            tickers = []
            if region:
                tickers.append(region)

            # Urgent Crucix items get BEARISH default (conflict/crisis news)
            sentiment = "BEARISH" if urgent else "NEUTRAL"
            confidence = 0.6 if urgent else 0.4

            try:
                conn.execute(
                    text(
                        "INSERT INTO news_articles "
                        "(dedup_hash, title, source, url, published_at, "
                        "tickers, sentiment, confidence) "
                        "VALUES (:h, :t, :s, :u, :p, :tk, :se, :c)"
                    ),
                    {
                        "h": dedup_hash,
                        "t": headline[:500],
                        "s": f"Crucix/{source}",
                        "u": url[:1000],
                        "p": published_at or datetime.now(timezone.utc),
                        "tk": tickers,
                        "se": sentiment,
                        "c": confidence,
                    },
                ...
                log.debug("Crucix news insert skipped: {e}", e=str(exc))

        return inserted

    def _ingest_ideas_and_delta(self, data: dict, conn: Any) -> int:
        """Ingest Crucix trade ideas and delta signals into signal_sources.

        Trade ideas become actionable signals with confidence scoring.
        Delta changes track what's moving between sweeps.
        """
        inserted = 0
        today = date.today()
        ...
        for idea in ideas:
            ...
            text_body = idea.get("text", "")
            idea_type = idea.get("type", "")  # long, short, hedge
            confidence = idea.get("confidence", "Low")
            horizon = idea.get("horizon", "")
            evidence = idea.get("evidence_class", "")
            ...
            signal_value = json.dumps({
                ...
                "calibration": idea.get("calibration", ""),
            })
            ...
            try:
                conn.execute(
                    text(
                        ...
                        "VALUES ('crucix_idea', :sid, :ticker, :sd, "
                        ":st, :sv, :ts)"
                    ),
                    {
                        "sid": f"crucix_{title[:50].lower().replace(' ', '_')}",
                        "ticker": idea_type.upper() if idea_type in ("long", "short") else "MACRO",
                        "sd": today,
                        "st": f"trade_idea_{idea_type}",
                        "sv": signal_value,
                        "ts": conf_val,
                    },
                )
                ...
                log.debug("Crucix idea insert skipped: {e}", e=str(exc))

        # ── Delta signals (changes between sweeps) ──
        ...
        for sig in delta_signals:
            ...
            try:
                sig_value = json.dumps(sig)
                source_key = sig.get("source", sig.get("key", "unknown"))
                conn.execute(
                    text(
                        "INSERT INTO signal_sources "
                        "(source_type, source_id, ticker, signal_date, "
                        "signal_type, signal_value, trust_score) "
                        "VALUES ('crucix_delta', :sid, 'MACRO', :sd, "
                        "'delta_change', :sv, 0.5)"
                    ),
                    {
                        "sid": f"crucix_delta_{source_key}",
                        "sd": today,
                        "sv": sig_value,
                    },
                )
                ...
                log.debug("Crucix delta insert skipped: {e}", e=str(exc))
